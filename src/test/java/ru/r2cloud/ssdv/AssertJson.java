...
public class AssertJson {
	...
	public static void assertObjectsEqual(String jsonFilename, Object actualObject, Class<?> clazz) {
		...
		Object expected;
		try (InputStreamReader reader = new InputStreamReader(AssertJson.class.getResourceAsStream("/" + jsonFilename), StandardCharsets.UTF_8)) {
	...
	private static void assertElements(StringBuilder message, String fieldName, Object expected, Object actual) {
		...
		if (expected.getClass().isPrimitive()) {
			...
			for (Method m : methods) {
				...
				try {
					Field field = expected.getClass().getDeclaredField(Character.toLowerCase(curField.charAt(0)) + curField.substring(1));
